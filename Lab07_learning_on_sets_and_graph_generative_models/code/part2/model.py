...
class Encoder(nn.Module):
    ...
    def forward(self, A, X, idx):
        ...
        A = torch.mm(torch.linalg.inv(D), A)
        ...
        H_out = torch.zeros(torch.max(idx).int().item()+1, H.size(1), device=H.device, requires_grad=False)
        H_out = H_out.scatter_add_(0, idx, H) 
...
class Decoder(nn.Module):
    ...
    def forward(self, X):
        ...
        T2 = self.fc_proj(T1).reshape((-1, self.n_nodes, self.n_nodes))
        A = torch.sigmoid((T2 + T2.transpose(1, 2)) / 2)
