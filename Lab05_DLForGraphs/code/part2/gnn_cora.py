...
class GNN(nn.Module):
    ...
    def forward(self, X, A):
        ############## Tasks 13
        Z1 = self.dropout(self.relu(self.fc1(torch.mm(A, X))))
        Z2 = self.relu(self.fc2(torch.mm(A, Z1)))
        Z3 = self.fc3(Z2)
        return F.log_softmax(Z3, dim=-1), Z2
...
adj = normalize_adjacency(adj.toarray()) # Normalize adjacency matrix
...
embeddings_test_array = embeddings_test.detach().cpu().numpy()

# Projects the emerging representations to two dimensions using t-SNE
tsne = TSNE(n_components=2)
embeddings_test_2d = tsne.fit_transform(embeddings_test_array)
